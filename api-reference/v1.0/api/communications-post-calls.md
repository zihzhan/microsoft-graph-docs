...
> **Note:** This example call needs the `Calls.Initiate.All` permission.
...
The values of authorization token, callback url, application id, application name, user id, user name and tenant id must be replaced along with the details obtained for `chatInfo` and `meetingInfo` with actual values to make the example work.
...
Meeting inside a channel requires specific details like thread id, messageid, and organizer details.

The values of authorization token, callback url, application id, application name, user id, user name and tenant id must be replaced along with the details for `chatInfo` and `meetingInfo` with actual values to make the example work.
