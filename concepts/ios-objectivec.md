...
This article describes the tasks required to get an access token from the [Azure AD v2.0 endpoint](converged-auth.md) and call Microsoft Graph. It walks you through the code inside the [Office 365 Connect Sample for iOS (SDK)](https://github.com/microsoftgraph/ios-objectivec-connect-sample) to explain the main concepts that you have to implement in an app that uses Microsoft Graph. It describes how to access Microsoft Graph by using the [Microsoft Graph SDK for iOS](https://github.com/microsoftgraph/msgraph-sdk-ios).
...
**uploadPictureToOneDrive** uploads the [user](/graph/api/resources/user?view=graph-rest-1.0)'s profile picture from their [user](/graph/api/resources/user?view=graph-rest-1.0) information and returns the web sharing Url to be embedded in the body of the email that is sent by the sample.
...
**getUserPicture** returns the [user](/graph/api/resources/user?view=graph-rest-1.0)'s profile picture if it is available.
   ```objectivec
...
This method gets the [user](/graph/api/resources/user?view=graph-rest-1.0) resource representing the authenticated user and caches the fields necessary to get the user's profile picture and send an email.
   ```objectivec
