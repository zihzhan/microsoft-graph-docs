...
- The contacts API lets you assign categories contacts, as well as events, messages, tasks, and group posts in a consistent way to enhance organization and disovery. In addition, you can [define a user's master list of categories](/graph/api/api/outlookuser_post_mastercategories?view=graph-rest-1.0), which can open up additional creative scenarios.
