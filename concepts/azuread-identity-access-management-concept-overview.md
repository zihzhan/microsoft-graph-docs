...
- To control access, you can check for [transitive membership](/graph/api/api/user_checkmembergroups?view=graph-rest-1.0) in a list of groups or get all the resources of a specified type (like user or group) from a list of [generic resource IDs](/graph/api/api/directoryobject_getbyids?view=graph-rest-1.0).
