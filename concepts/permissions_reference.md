...
#### Delegated
* _AppCatalog.ReadWrite.All_: [List all applications in catalog](/graph/api/api/teams_apps_list_published?view=graph-rest-beta) (`GET /beta/appCatalogs/teamsApps`)
* _AppCatalog.ReadWrite.All_: [Publish an app](/graph/api/api/teams_apps_publish?view=graph-rest-beta) (`POST /beta/appCatalogs/teamsApps`)
* _AppCatalog.ReadWrite.All_: [Update a published app](/graph/api/api/teams_apps_update_published?view=graph-rest-beta) (`PATCH /beta/appCatalogs/teamsApps/{id}`)
* _AppCatalog.ReadWrite.All_: [Remove a published app](/graph/api/api/teams_apps_remove_published?view=graph-rest-beta) (`DELETE /beta/appCatalogs/teamsApps/{id}`)
...
> **Note:** For request examples, see to [Create call](/graph/api/api/application_post_calls?view=graph-rest-beta).
...
They also exclusively control access to other directory resources like: [organizational contacts](/graph/api/resources/orgcontact?view=graph-rest-beta), [schema extension APIs](/graph/api/resources/schemaextension?view=graph-rest-beta), [Privileged Identity Management (PIM) APIs](/graph/api/resources/privilegedidentitymanagement_root?view=graph-rest-beta), as well as many of the resources and APIs listed under the **Azure Active Directory** node in the v1.0 and beta API reference documentation. These include administrative units, directory roles, directory settings, policy, and many more. 
...
The Files.ReadWrite.AppFolder delegated permission is only valid for personal accounts and is used for accessing the [App Root special folder](https://dev.onedrive.com/misc/appfolder.htm) with the OneDrive [Get special folder](/graph/api/api/drive_get_specialfolder?view=graph-rest-1.0) Microsoft Graph API.
...
Group permissions are also used to control access to [Microsoft Planner](/graph/api/resources/planner_overview?view=graph-rest-beta) resources and APIs. Only delegated permissions are supported for Microsoft Planner APIs; application permissions are not supported. Personal Microsoft accounts are not supported.
...
* _OnlineMeetings.Read.All_: Retrieve the properties and relationships of an [Online Meeting](/graph/api/api/onlinemeeting_get?view=graph-rest-beta) (`GET /beta/app/onlinemeetings/{id}`).
* _OnlineMeetings.ReadWrite.All_: Create an [Online Meeting](/graph/api/api/application_post_onlineMeetings?view=graph-rest-beta) (`POST /beta/app/onlinemeetings`).

> **Note**: Creating an [Online Meeting](/graph/api/api/application_post_onlineMeetings?view=graph-rest-beta) creates a meeting on behalf of a user specified in the request body, but does not show it on the user's Calendar.
