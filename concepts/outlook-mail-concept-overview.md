...
- Outlook categories are differentiated by name and color. Categories allow customers to tag messages to enhance organization and discovery. Apps can access and [define a user's master list of categories](/graph/api/api/outlookuser_post_mastercategories?view=graph-rest-1.0). More, that list is shared across Outlook messages,
