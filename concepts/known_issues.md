...
The current API to [create a chat thread](/graph/api/api/channel_post_chatthreads?view=graph-rest-beta) 
will be replaced with a richer API that is consistent with the schema for [listing channel messages](/graph/api/api/channel_list_messages?view=graph-rest-beta).
...
As an alternative, you can simply [get](/graph/api/api/contact_get?view=graph-rest-beta) the contact by specifying its ID as shown below,
...
The **comment** parameter for creating a reply or forward draft ([createReply](/graph/api/api/message_createreply?view=graph-rest-1.0),
[createReplyAll](/graph/api/api/message_createreplyall?view=graph-rest-1.0), [createForward](/graph/api/api/message_createforward?view=graph-rest-1.0))
