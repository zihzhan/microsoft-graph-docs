...
You can [create an open extension](/graph/api/api/opentypeextension_post_opentypeextension?view=graph-rest-1.0) in a resource instance and store custom data to it all in the same operation (note [known limitation](known_issues.md#extensions) for some of the supported resources).

You can subsequently [read](/graph/api/api/opentypeextension_get?view=graph-rest-1.0), [update](/graph/api/api/opentypeextension_update?view=graph-rest-1.0), or [delete](/graph/api/api/opentypeextension_delete?view=graph-rest-1.0)
...
Unlike open extensions, managing schema extension definitions ([list](/graph/api/api/schemaextension_list?view=graph-rest-1.0), [create](/graph/api/api/schemaextension_post_schemaextensions?view=graph-rest-1.0), [get](/graph/api/api/schemaextension_get?view=graph-rest-1.0), [update](/graph/api/api/schemaextension_update?view=graph-rest-1.0), and [delete](/graph/api/api/schemaextension_delete?view=graph-rest-1.0))
